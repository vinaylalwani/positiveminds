...
import { View, SafeAreaView, FlatList, Share, Dimensions, TouchableOpacity, Platform } from 'react-native'
...
import { getMusicApi } from "../../Services/API/MusicApi";
...
function Music({ navigation }) {
  ...
  const [musiData, setMusicData] = useState([])
  ...
  const fetchMusic = () => {
    ...
    getMusicApi(nextPageCall)
      .then(res => {
        // console.warn('music page res cur', res.data.cursor.nextCursor);
        // console.warn('data', res?.data?.data.length);
        

        if (res) {
          if (res?.data?.cursor?.nextCursor) {
            setMusicData(prevData => [...prevData, ...res?.data?.data]);
  ...
  return (
    <SafeAreaView style={Alignment.fill}>
      <View style={styles.container}>
        ...
        <FlatList
          ListHeaderComponent={musiData && musiData.length && renderListHeader}
          // nestedScrollEnabled
          data={musiData}
